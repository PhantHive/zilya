...
export default new Event('guildMemberAdd', async (member) => {
    ...
    let generalPath = path.join(
        __dirname,
        `../../assets/img/welcome/${themes[data.theme]['name']}`
    );
    ...
    background = await Canvas.loadImage(
        rand_back[Math.floor(Math.random() * rand_back.length)]
    );
    guildColor = '#ffffff';
