...
const updateRank = async (newState: VoiceState, timer: number) => {
    if (!newState.member) { console.log(`Error: no member found in the server ${newState.guild.name}`); return; }
    const data = await getUserDataRank(newState);
    if (!data) { console.log(`Error: no data found for the user ${newState.member.user.tag} in the server ${newState.guild.name}`); return; }
            ...
            if (data) {
                ...
                if (
                    ...
                    const voiceLog = new EmbedBuilder()
                        ...
                        .setDescription(
                            `**${newState.member.user.tag}** ${memberState} in/a voice channel.`
                        )
                        .addFields(
