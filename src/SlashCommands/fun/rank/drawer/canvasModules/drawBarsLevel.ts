...
class DrawBarsLevel extends TextFormatter {
	...
	public async drawBar(x: number, y: number, width: number, height: number, fillColor: string) {
		// Draw bar
	...
	public async drawXpBar() {
		...
		const nextLvlXpVocal = 25 * this.data.levelVocal ** 2 + 15 * this.data.levelVocal + 25;
		...
		this.ctx.drawImage(msgXpEmoji, this.canvas.width * 0.005, this.canvas.height * 0.64, 20, 20);
		this.ctx.drawImage(
			vocalXpEmoji,
			this.canvas.width * 0.000_2,
		...
		const xpMsgTxt = `${currentXpMsg}/${nextLvlXpMsg}`;
		...
		const xpVocalTxt = `${currentXpVocal}/${nextLvlXpVocal}`;
		...
		const levelMsgTxt = `Lvl: ${this.data.levelMsg}`;
		...
		const levelVocalTxt = `Lvl: ${this.data.levelVocal}`;
		...
		// ----------------- RANK -----------------
		const rankMsgTxt = `#${this.data.rankMsg}`;
		...
		const rankVocalTxt = `#${this.data.rankVocal}`;
