...
const drawShape = async (
    ...
    ctx.quadraticCurveTo(
        365 * 0.75,
        (canvas.height * 0.6) / 2 + canvas.height * 0.4,
        365,
        canvas.height * 0.4
    );
    ctx.quadraticCurveTo(
    ...
    // --------------------
    ctx.quadraticCurveTo(365 * 0.24, canvas.height * 0.01, 365 * 0.1, 0);
    // --------------------
    ctx.lineTo(0, 0);
    ...
    ctx.quadraticCurveTo(
        canvas.width * 0.81,
        (canvas.height * 0.6) / 2 + canvas.height * 0.45,
        canvas.width * 0.7,
        canvas.height
    );
    ...
    const background = await loadImage(
        path.join(
            __dirname,
            `../../assets/img/welcome/landscapes/landscape-2.jpg`
        )
    );
...
const drawXpBar = async (ctx, canvas, data: any): Promise<void> => {
    ...
    const nextLvlXpVocal =
        25 * data.levelVocal ** 2 + 15 * data.levelVocal + 25;
    ...
    drawBar(
        ...
        canvas.width * 0.04,
        canvas.height * 0.65,
        canvas.width * 0.33 * percentageMsg,
        17,
    ...
    drawBar(
        ...
        canvas.width * 0.04,
        canvas.height * 0.85,
        canvas.width * 0.33 * percentageVocal,
        17,
        'rgba(145,127,127,0.9)'
    );
...
const drawCard = async (
    ...
    const offsetY =
        (canvas.height * 0.6) / 2 + canvas.height * 0.4 - imageHeight / 2;
    ...
    ctx.fillStyle = '#ffffff';
    let font: [string, number] = getFontSize(
        ctx,
        canvas.width * 0.2,
