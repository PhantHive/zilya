...
const drawCard = async (ctx, canvas, profile) => {
    ...
    ctx.quadraticCurveTo(canvas.width * 0.5, canvas.height * 0.1, canvas.width * 0.4, canvas.height * 0.5);
    ctx.quadraticCurveTo(canvas.width * 0.32, canvas.height * 0.7, canvas.width * 0.1, canvas.height * 0.7);
    ...
    let font = await getFontSize(ctx, canvas.width * 0.4, profile.mostPlayedChamp, 200);
    ctx.font = font[0];
...
const drawStats = async (ctx, canvas, profile) => {
    ...
    ctx.drawImage(playerAvatar, canvas.width * 0.22 - (435/2), canvas.height * 0.18 - (435/2), 435, 435)
    ...
    ctx.fillText(profile.name, canvas.width - font[1] - canvas.width * 0.22 + 215  , canvas.height * 0.18 - 215 + actualHeight);
