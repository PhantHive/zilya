...
const isChannelValid = async (channel: string, configName: string) =>
    new Promise((resolve, reject) => {
        ...
        if (Number.isInteger(id) && id !== 0) {
            resolve(
                `Thank you! I will setup ${configName} message in this channel.`
            );
        } else {
            reject(
                `Sorry, I can't setup the ${configName} message in this channel. It is probably not a valid channel.`
            );
        }
    });
    ...
    return new Promise(async (resolve, reject) => {
        if (interaction.values[0] === 'manually') {
            ...
            await interaction.update({
                content:
                    'Please write the ID of the desired channel for welcome messages to appear.',
                components: [],
            });
...
const setTheme = async (data, interaction: ExtendedSelectMenuInteraction) => {
    ...
    new Promise((resolve, reject) => {
        ...
        if (choosedTheme >= 0 && choosedTheme <= 5) {
            ...
            data.save();
            resolve(
                `You choosed the theme: **${themeName}**, I will setup the welcome message with this theme.\n Proceeding to the next step...`
            );
        } else {
            reject(
                "Sorry, I can't setup the welcome message with this theme. Please try again."
            );
        }
    })
        .then(async (res: string) => {
...
const setColor = async (data, interaction: ExtendedSelectMenuInteraction) => {
    ...
    new Promise((resolve, reject) => {
        if (choosedColor !== '#000000') {
            ...
            data.save();
            resolve(
                `You choosed the color: **${choosedColor}**, I will setup the welcome message with this color.\n Proceeding to the next step...`
            );
        } else {
            reject(
                "Sorry, I can't setup the welcome message with this color. Please try again."
            );
        }
    })
        .then(async (res: string) => {
