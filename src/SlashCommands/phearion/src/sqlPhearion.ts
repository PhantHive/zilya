    ...
    async getBankInfos(userName: string) {
        ...
        try {
            ...
            await (await this.connection)
                .query(
                    `SELECT *FROM eco_accounts WHERE player_name = "${userName}" `
                )
                .then((result) => {
                    info = result[0][0];
                });
    ...
    async transferMoney(userName: string, amount: number) {
        try {
            ...
            await (
                ...
                `UPDATE eco_accounts SET money = ${amount}WHERE player_name = '${userName}'`
            );
    ...
    async addProperty(userName: string, property: string) {
        try {
            ...
            await (
                ...
                .query(`INSERT INTO luckperms_user_permissions (id, uuid, permission, value, server, world, expiry, contexts)
