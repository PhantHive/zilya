        ...
        new Promise(async (resolve, reject) => {
            ...
            if (!data) {
                ...
                await interaction
                    .editReply({
                        content:
                            `You need to authentify yourself.\n` +
                            `Please write in the minecraft server the following sentence: ***verify bank account ${randomNb}***`,
                    })
                    .then(() => {
                        const guild =
                            client.guilds.cache.get('717344084695580672');
                        const channel =
                            guild.channels.cache.get('732984129443987496');
                ...
                let daily = '✅';
                if (
                    (result['money'] as number) !== (data.pheaCoins as number)
                ) {
