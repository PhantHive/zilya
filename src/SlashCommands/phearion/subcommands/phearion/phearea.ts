...
export const pheareaSubCommand = new SubCommand({
    ...
    run: async ({ interaction }) => {
        const info = (interaction as ExtendedInteraction).options.get('info')
            .value as string;
        ...
        if (!data) {
            ...
            let row = new ActionRowBuilder().addComponents(buyArea, buyFriend);
            ...
            const filter: any = (i: ExtendedInteraction) =>
                (i.customId === 'buy_area' || i.customId === 'buy_friend') &&
            ...
            collector.on('collect', async (i: ExtendedInteraction) => {
                if (i.customId === 'buy_friend') {
                    ...
                    if (data.discoins < price) {
                        ...
                        await sql
                            .addProperty(data.mcNick, image_name)
                            .then(async () => {
                                data.discoins -= price;
                                ...
                                await i.editReply({
                                    content: `<a:stevedance:879145932451639306> Tu viens juste de débloquer une nouvelle zone: ***${image_name}*** !!! \n *(Reconnecte toi pour recevoir ton achat.)*`,
                                });
                            })
                            .catch(async (err) => {
                                await i.update({
                                    content: `**[❌]**: Erreur lors de l'achat de la propriété.`,
                                });
                            });
                    }
                }
