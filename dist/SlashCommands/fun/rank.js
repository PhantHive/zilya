...
const drawShape = async (ctx, canvas) => {
    ...
    const background = await (0, canvas_1.loadImage)(`${process.cwd()}/src/assets/img/welcome/landscapes/landscape-2.jpg`);
    ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
};
const drawBar = (ctx, x, y, width, height, fillColor) => {
    // Draw bar
    // if width is less than 0.002% of the canvas width, don't draw it
    if (width < 0.002 * ctx.canvas.width)
        return;
...
const drawXpBar = async (ctx, canvas, data) => {
    ...
    let xpMsgTxt = `${currentXpMsg}/${nextLvlXpMsg}`;
    font = getFontSize(ctx, (canvas.width * 0.35) * 0.37, xpMsgTxt, 24);
    font[0] = font[0].replace("normal", "bold");
    ctx.font = font[0];
    ctx.fillStyle = "#ffffff";
    ctx.textAlign = "center";
    ctx.fillText(xpMsgTxt, canvas.width * 0.2, canvas.height * 0.705);
    let xpVocalTxt = `${currentXpVocal}/${nextLvlXpVocal}`;
...
async function drawGlobalRank(ctx, canvas, data, interaction) {
    ...
    // top right
    ctx.textAlign = "left";
    let textWidth = canvas.width * 0.85 - font[1] / 2;
    ctx.fillText(globalRankTxt, textWidth, canvas.height * 0.42);
}
const drawCard = async (ctx, canvas, data, interaction) => {
    // draw global shape
    await drawShape(ctx, canvas);
    ctx.globalAlpha = 1;
    ctx.save();
    // draw the avatar
    let avatarCanvas = (0, canvas_1.createCanvas)(canvas.width, canvas.height);
    let avatarCtx = avatarCanvas.getContext("2d");
    const avatarUrl = interaction.member.displayAvatarURL({ extension: 'png', forceStatic: false, size: 2048 });
    let avatar;
    async function loadAvatar() {
        avatar = await (0, canvas_1.loadImage)(avatarUrl);
    }
    await loadAvatar();
    ...
    const serverIconUrl = interaction.guild.iconURL({ extension: 'png', forceStatic: false });
    let serverIcon;
    async function loadServerIcon() {
        serverIcon = await (0, canvas_1.loadImage)(serverIconUrl);
    }
    ...
    ctx.fillStyle = "#ffffff";
    let font = getFontSize(ctx, canvas.width * 0.2, `${interaction.user.username}`, 75);
    ctx.font = font[0];
    ...
    return new discord_js_1.AttachmentBuilder(canvas.toBuffer(), { name: 'rank_card.png' });
};
exports.default = new SlashCommand_1.SlashCommand({
    ...
    run: async ({ interaction }) => {
        ...
        // creating context
        const canvas = (0, canvas_1.createCanvas)(670, 270);
