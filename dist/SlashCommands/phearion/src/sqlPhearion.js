...
class sqlPhearion {
    ...
    async transferMoney(userName, amount) {
        try {
            // start transaction
            await (await this.connection).beginTransaction();
    ...
    async addProperty(userName, property) {
        try {
            // start transaction
...
exports.sqlPhearion = sqlPhearion;
