...
exports.default = new Event_1.Event('voiceStateUpdate', async (oldState, newState) => {
    ...
    if (newState.channelId && !oldState.channelId) {
        setInterval(async () => {
            if (!newState.channelId || ((newState.channelId !== oldState.channelId) && oldState.channelId))
        ...
        new Promise(async (resolve) => {
            if (data) {
                ...
                if (logger !== undefined && data.notifType !== "no_voice_logs") {
                    ...
                    const voiceLog = new discord_js_1.EmbedBuilder()
                        ...
                        .addFields({ name: "Voice channel TAG", value: tagValue }, {
                        name: 'All IDs', value: fieldComment
                    })
                        .setColor(color)
                        .setTimestamp()
                        .setFooter({ text: `by PhearionNetwork. Sever: ${newState.guild.name}`, iconURL: index_1.client.user.displayAvatarURL() });
